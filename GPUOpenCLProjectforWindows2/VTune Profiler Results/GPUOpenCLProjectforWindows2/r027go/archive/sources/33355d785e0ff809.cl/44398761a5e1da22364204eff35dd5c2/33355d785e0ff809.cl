...
const int mDim, const int pDim, const int nDim, global float* biases)
{
    ...
    __local float Asub[2*TS][2*TS];
    __local float Bsub[2*TS][2*TS];
    __local float Csub[2*TS][2*TS];
    __local float Dsub[2*TS][2*TS];
    __local float Esub[2*TS][2*TS];
    __local float Fsub[2*TS][2*TS];
    __local float Gsub[2*TS][2*TS];
    __local float Hsub[2*TS][2*TS];
    __local float Isub[2*TS][2*TS];
    ...
    for (int t=0; t<numTiles; t++) {
        ...
        Csub[row][col] = matrixA[tiledCol + globalRow*pDim];
        Dsub[row][col] = matrixB[globalCol+ tiledRow*nDim];
        Esub[row][col] = matrixA[tiledCol + globalRow*pDim];
        Fsub[row][col] = matrixB[globalCol+ tiledRow*nDim];
        Gsub[row][col] = matrixA[tiledCol + globalRow*pDim];
        Hsub[row][col] = matrixB[globalCol+ tiledRow*nDim];
        Isub[row][col] = matrixB[globalCol+ tiledRow*nDim];
