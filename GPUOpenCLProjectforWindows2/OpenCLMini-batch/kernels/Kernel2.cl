...
const int mDim, const int pDim, const int nDim, global float* biases)
{
    ...
    for (int t=0; t<blocks; t++) {
        ...
        tempA[row][col] = matrixA[tempCol + globalRow*pDim];
        tempB[row][col] = matrixB[globalCol+ tempRow*nDim];
        barrier(CLK_LOCAL_MEM_FENCE);
 
        for (int k=0; k<WGS; k++) {
            temp += tempA[row][k] * tempB[k][col];
        }
...
const int mDim, const int pDim, const int nDim, global float* biases)
{
    ...
    __local float tempA[WGS][WGS];
    __local float tempB[WGS][WGS];
...
const int mDim, const int pDim, const int nDim, global float* matrixD)
{
    ...
    for (int t=0; t<blocks; t++) {
        ...
        tempA[row][col] = matrixA[tempCol*mDim + globalRow];
        tempB[row][col] = matrixB[globalCol+ tempRow*nDim];
...
,const int mDim, const int pDim, const int nDim, global float* biases, const float offset)
{
    ...
    for (int t=0; t<blocks; t++) {
        ...
        const int tempCol = WGS*t + col;
        tempA[row][col] = matrixA[tempCol + globalRow*pDim];
        tempB[row][col] = matrixB[globalCol*pDim+ tempRow];
