...
const int mDim, const int pDim, const int nDim, global float* matrixD)
{
 // Thread identifiers
    ...
    // Store the final results in C
    for (int w=0; w<WPT; ++w) {
        matrixC[(globalCol + w*RTS) + globalRow*nDim] = acc[w]* matrixD[(globalCol + w*RTS) + globalRow*nDim] * (1.0 - matrixD[(globalCol + w*RTS) + globalRow*nDim]);
    }
}
...
const int mDim, const int pDim, const int nDim, global float* matrixD)
{
  // Thread identifiers
    ...
    for (int w=0; w<WPT; ++w) {
        matrixC[(globalCol + w*RTS) + globalRow*nDim] = acc[w]*(1 - pow(matrixD[(globalCol + w*RTS) + globalRow*nDim],2));
    }
}
...
// Thread identifiers
    ...
    // Store the final result in C
    for (int w=0; w<WPT; ++w) {
        matrixC[(globalCol + w*RTS) + globalRow*nDim] = acc[w]*fmax(matrixD[(globalCol + w*RTS) + globalRow*nDim],0);
    }
}
...
// Thread identifiers
    ...
    // Store the final result in C
    for (int w=0; w<WPT; ++w) {
        matrixC[(globalCol + w*RTS) + globalRow*nDim] = matrixC[(globalCol + w*RTS) + globalRow*nDim] - - clamp(offset*acc[w]/(float)pDim,-0.005f,0.005f);
    } 
}